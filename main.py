...
async def start(message: Message):
    ...
    with db.connection() as conn:
        cursor = conn.cursor()
        cursor.execute("UPDATE users SET chat_id=? WHERE id=?", (message.chat.id, user_id))
        conn.commit()
...
async def handle_all_messages(message: Message):
    try:
        ...
        # Якщо не зареєстрований - пропонуємо реєстрацію (тільки якщо немає активної дії)
