...
# =========================
UKRAINE_TZ = timedelta(hours=3)

def get_ukraine_time():
    """Повертає поточний час в Україні (UTC+3)"""
    return datetime.utcnow() + UKRAINE_TZ
...
async def start(message: Message):
    ...
    if user and user['is_registered'] == 1:
        ...
        await message.answer("🛡️ **Головне меню**", reply_markup=main_menu(message.from_user.id), parse_mode="Markdown")
        return
...
async def handle_all_messages(message: Message):
    try:
        ...
        if user and not user['is_registered'] and message.text:
            # Перевіряємо чи немає вже активної реєстрації
        ...
        # РЕЄСТРАЦІЯ (тільки через /start, але якщо хтось ввів нік)
        ...
        if action["type"] == "create_event_date":
            try:
                # Використовуємо Український час для перевірки
