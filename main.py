...
def register_user(message: Message):
    ...
    with db.connection() as conn:
        ...
        if not result:
            ...
            cursor.execute("""
                INSERT INTO users(id, username, clan_name, role, role_level, last_online, registered_at, chat_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                user_id, username, None, role, level, int(time.time()), int(time.time()), 0
            ))
            conn.commit()
...
# =========================
# STAFF LIST (чистий, без зайвих фраз)
# =========================
...
async def event_notifications():
    ...
    while True:
        try:
            ...
            # ПЕРЕКЛИЧКА О 10:00 (тільки в чати)
